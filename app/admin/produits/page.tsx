    ...
    <>
        ...
        <div style={{ ...HEADER_ZONE, padding: "1.5rem 1.75rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1.25rem" }}>
            ...
            <button
              type="button"
              onClick={openCreate}
              style={{ padding: "5px 14px", borderRadius: "8px", border: "1px solid rgba(200,162,77,0.30)", color: "rgba(200,162,77,0.60)", fontSize: "0.68rem", background: "rgba(6,14,7,0.40)", backdropFilter: "blur(8px)", letterSpacing: "0.04em" }}
            >
              + Nouveau produit
            </button>
          </div>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              ...
              {!loading && products.length === 0 && (
                <tr><td colSpan={6} style={{ padding: "3.5rem 1.75rem", textAlign: "center", color: "rgba(232,224,208,0.22)", fontSize: "0.8rem" }}>Aucun produit — la table products n&apos;est peut-être pas encore créée dans Supabase.</td></tr>
              )}
              {!loading && products.map((p, i) => (
                <tr key={p.id} style={{ borderBottom: i < products.length - 1 ? "1px solid rgba(200,162,77,0.07)" : "none" }}>
                  <td style={{ padding: "1.1rem 1.75rem" }}>
                    ...
                    <p style={{ color: "rgba(232,224,208,0.28)", fontSize: "0.70rem" }}>{p.tagline}</p>
                  </td>
                  ...
                  <td style={{ padding: "1.1rem 1.75rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                      <button type="button" onClick={() => updateStock(p, -1)} style={{ width: 24, height: 24, borderRadius: "6px", border: "1px solid rgba(200,162,77,0.22)", color: "rgba(232,224,208,0.42)", fontSize: "0.9rem", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                      <span style={{ minWidth: "2rem", textAlign: "center", fontSize: "0.82rem", fontWeight: 500, color: p.stock_quantity <= 2 ? "rgba(200,80,80,0.85)" : "rgba(232,224,208,0.82)" }}>
                        {p.stock_quantity}
                      </span>
                      <button type="button" onClick={() => updateStock(p, 1)} style={{ width: 24, height: 24, borderRadius: "6px", border: "1px solid rgba(200,162,77,0.22)", color: "rgba(232,224,208,0.42)", fontSize: "0.9rem", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                  ...
                  <td style={{ padding: "1.1rem 1.75rem" }}>
                    <button type="button" onClick={() => openEdit(p)} style={{ fontSize: "0.72rem", color: "rgba(200,162,77,0.50)", letterSpacing: "0.05em" }}>Modifier</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {modal && (
        <div
          ...
          style={{ background: "rgba(4,10,5,0.88)", backdropFilter: "blur(6px)" }}
        >
          <div
            style={{
              width: "100%",
              maxWidth: "500px",
              background: "rgba(6,14,7,0.72)",
          ...
          >
            ...
            <div style={{ ...HEADER_ZONE, padding: "1.25rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button type="button" onClick={() => setModal(null)} style={{ color: "rgba(232,224,208,0.28)", fontSize: "1.2rem" }}>×</button>
            </div>
            <div style={SEP} />
            ...
            <form onSubmit={handleSave} style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <Field id="f-name" label="Nom" value={editing.name ?? ""} onChange={(v) => setEditing((p) => ({ ...p, name: v }))} required />
              <Field id="f-tagline" label="Tagline" value={editing.tagline ?? ""} onChange={(v) => setEditing((p) => ({ ...p, tagline: v }))} />
              <div>
              ...
              <div>
                <label htmlFor="f-cat" className="block font-serif uppercase mb-1.5" style={LABEL}>Catégorie</label>
                <select id="f-cat" value={editing.category ?? "punch"} onChange={(e) => setEditing((p) => ({ ...p, category: e.target.value as "punch" | "coffret" }))} className="focus:outline-none" style={INPUT}>
                  <option value="punch" className="bg-charcoal">Punch</option>
                  <option value="coffret" className="bg-charcoal">Coffret</option>
                </select>
              </div>
              <div style={{ display: "flex", gap: "1.5rem" }}>
                <label htmlFor="f-active" style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.80rem", color: "rgba(232,224,208,0.62)", cursor: "pointer" }}>
                  <input id="f-active" type="checkbox" checked={editing.is_active ?? true} onChange={(e) => setEditing((p) => ({ ...p, is_active: e.target.checked }))} className="accent-gold" />
                  Actif
                </label>
                <label htmlFor="f-featured" style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.80rem", color: "rgba(232,224,208,0.62)", cursor: "pointer" }}>
                  <input id="f-featured" type="checkbox" checked={editing.is_featured ?? false} onChange={(e) => setEditing((p) => ({ ...p, is_featured: e.target.checked }))} className="accent-gold" />
                  Mis en avant
                </label>
              </div>
              {error && <p style={{ color: "rgba(220,80,80,0.85)", fontSize: "0.80rem" }}>{error}</p>}
              <div style={{ height: "1px", background: "linear-gradient(90deg, transparent, rgba(200,162,77,0.12), transparent)" }} />
              <div style={{ display: "flex", gap: "0.75rem" }}>
                <button type="button" onClick={() => setModal(null)} style={{ flex: 1, padding: "10px 0", borderRadius: "9px", border: "1px solid rgba(200,162,77,0.18)", color: "rgba(232,224,208,0.38)", fontSize: "0.78rem" }}>Annuler</button>
...
function Field({ id, label, value, onChange, required, disabled, type = "text" }: {
  ...
  return (
    <div>
      <label htmlFor={id} className="block font-serif uppercase mb-1.5" style={{ fontSize: "0.50rem", letterSpacing: "0.22em", color: "rgba(200,162,77,0.42)" }}>{label}</label>
      <input id={id} type={type} value={value} onChange={(e) => onChange(e.target.value)} required={required} disabled={disabled}
        ...
        style={{ background: "rgba(6,14,7,0.60)", border: "1px solid rgba(200,162,77,0.22)", borderRadius: "9px", color: "rgba(232,224,208,0.85)", fontSize: "0.82rem", width: "100%", padding: "9px 12px" }} />
    </div>
  );
}
